...
//生成子密钥  
int DES_MakeSubKeys(ElemType key[],ElemType subKeys[16][60]){  
    ElemType temp[60];  
    int cnt;  
...
//加密单个分组  
int DES_EncryptBlock(ElemType plainBlock[], ElemType subKeys[16][60], ElemType cipherBlock[]){  
    ElemType plainBits[70];  
    ElemType copyRight[60];  
    int cnt;  
...
//解密单个分组  
int DES_DecryptBlock(ElemType cipherBlock[], ElemType subKeys[16][60],ElemType plainBlock[]){  
    ElemType cipherBits[70];  
    ElemType copyRight[60];  
    int cnt;  
...
int DES_Encrypt(char *plainFile, char *keyStr,char *cipherFile){  
    FILE *plain,*cipher;  
    int count;  
    ElemType plainBlock[10],cipherBlock[10],keyBlock[10];  
    ElemType bKey[70];  
    ElemType subKeys[16][60];  
    if((plain = fopen(plainFile,"rb")) == NULL){  
        return PLAIN_FILE_OPEN_ERROR;  
    }     
    if((cipher = fopen(cipherFile,"w")) == NULL){  
